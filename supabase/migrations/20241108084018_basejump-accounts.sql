...
$$
    BEGIN
        ...
        IF NOT EXISTS(SELECT 1
            ...
            CREATE TYPE basejump.account_role AS ENUM ('super', 'owner', 'member');
        end if;
    end;
$$;
...
begin
    ...
    then
        generated_user_name := new.raw_user_meta_data->>'name';
    else
    ...
    insert into basejump.accounts (name, primary_owner_user_id, personal_account, id, public_metadata)
    values (
        generated_user_name,
        NEW.id, 
        true, 
        ...
        jsonb_build_object(
            ...
            'image', new.raw_user_meta_data->>'avatar'
        )
...
create or replace function basejump.is_super_user(account_id uuid)
    returns boolean
    language sql
    security definer
    set search_path = public
...
select exists(
               ...
               from basejump.account_user wu
               where wu.user_id = auth.uid()
                 and wu.account_id = $1 and wu.account_role = 'super'
           );
$$;

grant execute on function basejump.is_super_user(uuid) to authenticated;
...
create policy "Super users can view all accounts" on basejump.account_user
    for select
    to authenticated
    using (
...
create policy "Account users can be deleted by owners except primary account owner" on basejump.account_user
    ...
    using (
        ...
        OR
        (basejump.has_role_on_account(account_id, 'super') = true)
        AND
...
create policy "All accounts are viewable by super users" on basejump.accounts
    for select
    to authenticated
    using (
        basejump.is_super_user(auth.uid()) = true
...
-- Super account can create account with any role
create policy "Any accounts can be created by super user" on basejump.accounts
    for insert
    to authenticated
    with check (
        basejump.is_super_user(auth.uid()) = true
...
-- Super account can create account with any role
create policy "Any accounts can be updated by super user" on basejump.accounts
    for update
    to authenticated
    using (
        basejump.is_super_user(auth.uid()) = true
...
-- Super account can create account with any role
create policy "Any accounts can be deleted by super user" on basejump.accounts
    for delete
    to authenticated
    using (
        basejump.is_super_user(auth.uid()) = true
...
from basejump.account_user wu
         join basejump.accounts a on a.id = wu.account_id
where wu.user_id = auth.uid() or basejump.is_super_user(auth.uid()) = true;
$$;
...
BEGIN
    ...
    if current_user IN ('anon', 'authenticated') and basejump.is_super_user(auth.uid()) = false and
